...
namespace Better.Tweens.Runtime
{
    public static class TargetTweenExtensions
    {
        ...
        internal static IEnumerable<TTween> Tweens<TTween, TTarget, TValue, TValueOptions>(this IEnumerable<TTarget> self, TValueOptions options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, TValue, TValueOptions>, new()
            where TTarget : class
        {
            if (self == null)
            {
                ...
                return Array.Empty<TTween>();
            }

            var tweens = new List<TTween>();
            foreach (var target in self)
            {
                var tween = target.Tween<TTween, TTarget, TValue, TValueOptions>(options, optionsMode);
                tweens.Add(tween);
            }
        ...
        internal static IEnumerable<TTween> Tweens<TTween, TTarget, TValue>(this IEnumerable<TTarget> self, TValue options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, TValue>, new()
            where TTarget : class
        {
            return self.Tweens<TTween, TTarget, TValue, TValue>(options, optionsMode);
        }

        internal static IEnumerable<TTween> Tweens<TTween, TTarget>(this IEnumerable<TTarget> self, TTarget options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget>, new()
            where TTarget : class
        {
            return self.Tweens<TTween, TTarget, TTarget, TTarget>(options, optionsMode);
        }

        internal static IEnumerable<TTween> TweensColor<TTween, TTarget>(this IEnumerable<TTarget> self, Color options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Color, ColorOptions>, new()
            where TTarget : class
        {
            var colorOptions = new CustomColorOptions(options);
            return self.Tweens<TTween, TTarget, Color, ColorOptions>(colorOptions, optionsMode);
        }

        internal static IEnumerable<TTween> TweensVector2<TTween, TTarget>(this IEnumerable<TTarget> self, float options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Vector2>, new()
            where TTarget : class
        {
            var vectorOptions = Vector2.one * options;
            return self.Tweens<TTween, TTarget, Vector2>(vectorOptions, optionsMode);
        }

        internal static IEnumerable<TTween> TweensVector2Int<TTween, TTarget>(this IEnumerable<TTarget> self, int options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Vector2Int>, new()
            where TTarget : class
        {
            var vectorOptions = Vector2Int.one * options;
            return self.Tweens<TTween, TTarget, Vector2Int>(vectorOptions, optionsMode);
        }

        internal static IEnumerable<TTween> TweensVector3<TTween, TTarget>(this IEnumerable<TTarget> self, float options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Vector3>, new()
            where TTarget : class
        {
            var vectorOptions = Vector3.one * options;
            return self.Tweens<TTween, TTarget, Vector3>(vectorOptions, optionsMode);
        }

        internal static IEnumerable<TTween> TweensVector3Int<TTween, TTarget>(this IEnumerable<TTarget> self, int options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Vector3Int>, new()
            where TTarget : class
        {
            var vectorOptions = Vector3Int.one * options;
            return self.Tweens<TTween, TTarget, Vector3Int>(vectorOptions, optionsMode);
        }

        internal static IEnumerable<TTween> TweensVector4<TTween, TTarget>(this IEnumerable<TTarget> self, float options, OptionsMode optionsMode)
            where TTween : TargetTween<TTarget, Vector4>, new()
            where TTarget : class
        {
            var vectorOptions = Vector4.one * options;
            return self.Tweens<TTween, TTarget, Vector4>(vectorOptions, optionsMode);
        }
    }
}